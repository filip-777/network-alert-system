...
public class InMemoryDependencyRepository implements DependencyRepository {
  ...
  @Override
  public void addDependencyServiceToService(
    String serviceName,
    String dependencyName
  ) {
    if (InMemoryDatabase.DATABASE.containsKey(serviceName)) {
      boolean filteredDependencyName = InMemoryDatabase
        .DATABASE.get(serviceName)
        .stream()
      ...
      throw new NoSuchElementException(
        "There is no service with name " +
        serviceName +
        " to which can be added to the dependency service"
      );
    }
  }
  ...
  public List<String> getDependencies(String serviceName) {
    return Optional
      .ofNullable(InMemoryDatabase.DATABASE.get(serviceName))
